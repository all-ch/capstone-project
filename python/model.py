...
def compute_word_level_topic_score(document_text, topic_vector) -> list:
    ...
    for word in words:
        ...
        word_topic_score = cosine_similarity(
            word_embedding.reshape(1, -1), topic_vector.reshape(1, -1)
        )[0][0]
...
if __name__ == "__main__":
    ...
    religious_score = compute_word_level_topic_score(religious_text, religion_axis)
    non_religious_score = compute_word_level_topic_score(
        non_religious_text, religion_axis
    )
    ...
    axes[0].hist(
        religious_score, bins=30, alpha=0.7, color="steelblue", edgecolor="black"
    )
    axes[0].set_title(
    ...
    axes[0].axvline(
        np.mean(religious_score),
        color="red",
        linestyle="--",
        label=f"Mean: {np.mean(religious_score):.3f}",
    )
    axes[0].legend()
    ...
    axes[1].hist(
        non_religious_score, bins=30, alpha=0.7, color="coral", edgecolor="black"
    )
    axes[1].set_title(
    ...
    axes[1].axvline(
        np.mean(non_religious_score),
        color="red",
        linestyle="--",
        label=f"Mean: {np.mean(non_religious_score):.3f}",
    )
    axes[1].legend()
