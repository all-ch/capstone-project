...
def generate_anchor_embedding(pos_text: str, neg_text: str) -> tuple[np.ndarray, np.ndarray]:
...
def calculate_topic_vector(pos_embeddings: np.ndarray, neg_embeddings: np.ndarray) -> np.ndarray:
...
def compute_speech_topic_score(document_text: str, topic_vector: np.ndarray) -> float:
    # break the document into sentences, get the embedding for each sentence, and calculate the cosine similarity between each sentence embedding and the topic vector. Then, average the cosine similarity scores across all sentences to get an overall topic score for the document.
    doc = nlp(document_text)
    sentence_embeddings = model.encode([sent.text for sent in doc.sents])
    topic_score = cosine_similarity(sentence_embeddings, topic_vector.reshape(1, -1)).mean()
    return topic_score


# TA suggested yearly scores for each topic
def compute_yearly_topic_scores(conference_data: pd.DataFrame, topic_vector: np.ndarray) -> dict:
    # group the data by year, compute the average topic score for each year, and return a dictionary with years as keys and average topic scores as values
    yearly_topic_score = {}
...
def compute_word_level_topic_score(document_text, topic_vector) -> list:
    # break the document into words, get the embedding for each word, and calculate the cosine similarity between each word embedding and the topic vector. Return a list of cosine similarity scores for each word in the document.
    words = document_text.split() # note: super simple way to split into words, could be improved with better tokenization (e.g. using spacy)
    word_scores = []
    for word in words:
...
if __name__ == "__main__":
    ...
    # Maybe, change this part to use a smoothing spline instead of a linear regression line, since the relationship may not be linear. But for now, let's just do a linear regression line for simplicity.
